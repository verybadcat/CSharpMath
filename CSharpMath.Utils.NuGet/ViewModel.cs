...
namespace CSharpMath.Utils.NuGet {
  public class ViewModel : INotifyPropertyChanged {
    ...
    public void Initialize() {
      using (var s = new FileStream(App.ReleaseData, FileMode.Open)) {
        ...
        PackageRequireLicenseAcceptance =
          bool.Parse(GetValue(nameof(PackageRequireLicenseAcceptance)));
    ...
    public bool GeneratePackageOnBuild
      { get => _packOnBuild_Release; set => Set(_packOnBuild_Release = value); }
    string _version;
    ...
    public string PackageReleaseNotes
      { get => _releaseNotes; set => Set(_releaseNotes = value); }
    string _repoType;
    ...
    public string RepositoryBranch
      { get => _repoBranch; set => Set(_repoBranch = value); }
    string _repoCommit;
    public string RepositoryCommit
      { get => _repoCommit; set => Set(_repoCommit = value); }
    string _packageURL;
    public string PackageProjectUrl
      { get => _packageURL; set => Set(_packageURL = value); }
    bool _licenseRequired;
    public bool PackageRequireLicenseAcceptance
      { get => _licenseRequired; set => Set(_licenseRequired = value); }
    string _packageLicenseExpression;
    public string PackageLicenseExpression
      { get => _packageLicenseExpression; set => Set(_packageLicenseExpression = value); }
    string _packageIcon;
    ...
    public void SaveFile() {
      using (var file = new FileStream(App.ReleaseData, FileMode.Open)) {
        ...
        SetValue(nameof(GeneratePackageOnBuild),
                 GeneratePackageOnBuild.ToString().ToLowerInvariant());
        ((XmlElement)global.GetElementsByTagName(nameof(GeneratePackageOnBuild))[0])
          .SetAttribute("Condition", "'$(Configuration)' == 'Release'");
        ...
        SetValue(nameof(PackageRequireLicenseAcceptance),
          PackageRequireLicenseAcceptance.ToString().ToLowerInvariant());
